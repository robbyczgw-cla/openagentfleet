...
silently substitute Web Search Plus, Hound, or stale knowledge. See the
...
Both toggles are persisted independently. Reading status is side-effect-free;
first use of an enabled connector may download its pinned package through
`uvx`. A connector is not an Agent tool merely because its global toggle is on:
the Agent's `mcp_ids` list controls actual injection into that Agent's lead
run.
...
absolute bundled `uvx` path resolved by the launcher probe; source/development
runs may resolve `uvx` from their configured environment instead:
...
3.6.0](https://pypi.org/project/web-search-plus-mcp/3.6.0/) and [Hound MCP
13.1.2](https://pypi.org/project/hound-mcp/13.1.2/). Their upstream source
references are [web-search-plus-mcp](https://github.com/robbyczgw-cla/web-search-plus-mcp)
and [master-fetch/Hound](https://github.com/dondai44423/master-fetch).

`uvx` may populate its own package cache on first use. This is expected and is
why status calls this state **launcher ready**, not connector verified. It only
means the exact launch mechanism is available; a clean cache or offline first
use can still fail while fetching or initializing the package. The application
does not silently install either connector at first launch.

## Measured MCP readiness

Development QA measured successful MCP `initialize` handshakes for both pinned
connectors. That measurement is not reused as a runtime health claim: status
checks local launcher availability, while the selected lead harness performs
MCP startup and `initialize` for the actual run. The protocol
lifecycle is described by the [official MCP lifecycle specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/lifecycle).

Both connectors run as local stdio MCPs. An optional HTTP bridge, if enabled in
...
## No silent WSP-Hound bridge

Web Search Plus and Hound remain independent MCP servers. Enabling Hound does
not rewrite Web Search Plus configuration, and enabling Web Search Plus does
not start or select Hound. There is no silent WSP-to-Hound bridge in the
first-install architecture. If a future explicit bridge is added, it requires
its own visible setting, compatibility check, and QA evidence; it must not be
inferred from the two independent toggles.
...
`web-search-plus` or `hound`. Unknown IDs fail before a run instead of being
...
per-Agent Hound or Web Search Plus selection. It does not mean that source
