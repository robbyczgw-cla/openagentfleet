...
uv_binary="${OPENAGENTFLEET_UV_BINARY:-$(command -v uv || true)}"
uvx_binary="${OPENAGENTFLEET_UVX_BINARY:-$(command -v uvx || true)}"
...
opencode_binary="${OPENAGENTFLEET_OPENCODE_BINARY:-$(command -v opencode || true)}"
...
chmod 755 "$uv_path" "$uvx_path" "$opencode_path"
